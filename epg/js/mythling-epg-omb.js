...
epgApp.controller('EpgController', 
  ...
  var guideInterval = params.guideInterval ? parseInt(params.guideInterval) : 8; // hours
  var guideHistory = params.guideHistory ? parseInt(params.guideHistory) : 0; // hours (must be less than interval)
  $scope.bufferSize = params.bufferSize ? parseInt(params.bufferSize) : 3; // screen widths
  var awaitPrime = params.awaitPrime ? params.awaitPrime == 'true' : true; // whether to disable mobile scroll until loaded
  ...
  $scope.setPosition = function(offset) {
    ...
    if (newCurDate.getTime() != $scope.guideData.curDate.getTime()) {
      ...
      if (!$scope.guideData.busy) {
        var formatted = $filter('date')($scope.guideData.curDate, 'EEE MMM d');
        // TODO: updating DOM a no-no
        document.getElementById('calendarInput').value = formatted;
      }
    }
  };
...
epgApp.controller('EpgModalController', ['$scope', '$modalInstance', function($scope, $modalInstance) {
    ...
    link: function link(scope, elem, attrs) {
      elem.bind('click', function() {
        if (scope.popElem != elem) {
          ...
          var place = 'top';
          if (viewportOffset.top < (document.documentElement.clientHeight - viewportOffset.bottom))
            place = 'bottom';
          if (viewportOffset.left < 100)
            place = 'right';
          if ((document.documentElement.clientWidth - viewportOffset.right) < 100)
            place = 'left';
...
epgApp.factory('GuideData', ['$http', '$timeout', '$window', '$filter', function($http, $timeout, $window, $filter) {
  var GuideData = function(startDate, slotWidth, intervalHours, awaitPrime, historyHours, channelGroupId, mythlingServices, demoMode) {
    ...
    this.awaitPrime = awaitPrime; // only initially
  ...
  GuideData.prototype.setStartTime = function(startTime) {
    ...
    this.midnight = new Date(this.zeroTime.getTime() + 24*3600000);
    this.midnight.setHours(0);
    this.midnight.setMinutes(0);
    this.tomorrow = new Date(this.midnight.getTime() + 24*3600000);
    this.timeslots = [];
    
    this.index = 10;
  };
  
  GuideData.prototype.nextPage = function(programId) {
    if (this.busy)
      return;
    this.busy = true;
    ...
    // set immediately and off-thread
    document.getElementById('calendarInput').value = 'Loading...';
    $timeout(function() {
      // going through model screws up bootstrap calendar directive
      document.getElementById('calendarInput').value = 'Loading...';
    }, 5);
    
    if (this.awaitPrime) {
      var calInput = document.getElementById('calendarInput');
      angular.element(calInput).addClass('input-warn');
    }
    $http.get(url).success(function(data) {
      ...
      // TODO no-no
      var calInput = document.getElementById('calendarInput');
      angular.element(calInput).removeClass('input-warn');
      calInput.value = $filter('date')(this.curDate, 'EEE MMM d');
      this.busy = false;
...
epgCalendar.controller('EpgCalController', ['$scope', '$http', function($scope, $timeout) {
  ...
  $scope.currentDate = function(newValue) {
    if (newValue) {
      ...
      $scope.guideData.setStartTime(newValue);
